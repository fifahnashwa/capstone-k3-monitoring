...
class ViolationController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        if (!empty($validated['image_data'])) {
            try {
                $jpeg     = base64_decode($validated['image_data']);
                $filename = 'violations/viol_' . now()->format('Ymd_His_') . uniqid() . '.jpg';
                // Pastikan direktori violations ada dan bisa ditulis
                Storage::disk('public')->makeDirectory('violations');
                Storage::disk('public')->put($filename, $jpeg);
                $imagePath = $filename;
            } catch (\Exception $e) {
                Log::warning("Gagal menyimpan screenshot violation: {$e->getMessage()}");
            }
        }
    ...
    /**
     * PATCH /api/violations/patch-person-name — Retroactively set person_name
     * untuk violations terakhir kamera ini yang belum dikenali (dari detection worker).
     */
    public function patchPersonName(Request $request): JsonResponse
    {
        $validated = $request->validate([
        ...
        $updated = Violation::where('camera_id', $validated['camera_id'])
            ->whereNull('person_name')
            ->where('detected_at', '>=', now()->subMinutes(5))
            ->update(['person_name' => $validated['person_name']]);
    ...
    public function destroy(Request $request, Violation $violation): JsonResponse
    {
        ...
        if ($violation->image_path) {
            \Illuminate\Support\Facades\Storage::disk('public')->delete($violation->image_path);
        }

        // Hapus relasi dulu agar tidak terkena foreign key constraint
        \DB::table('violation_notifications')->where('violation_id', $violationId)->delete();
        ...
        ActivityLog::create([
            ...
            'description' => "Manager menghapus pelanggaran #{$violationId}.",
        ]);

        return response()->json(['message' => "Pelanggaran #{$violationId} berhasil dihapus."]);
    }
