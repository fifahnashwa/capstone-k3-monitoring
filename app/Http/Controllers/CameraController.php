...
class CameraController extends Controller
{
    ...
    /**
     * POST /api/cameras/upload-model — Upload file model .onnx.
     */
    public function uploadModel(Request $request): JsonResponse
    {
        $request->validate([
            'model' => 'required|file|mimes:onnx|max:512000',
        ]);

        $path = $this->cameraService->uploadModel($request->file('model'));
    ...
    public function dashboardStats(Request $request): JsonResponse
    {
        ...
        // Active workers (distinct person detections in last 5 minutes)
        $activeWorkers = Violation::where('detected_at', '>=', now()->subMinutes(5))
