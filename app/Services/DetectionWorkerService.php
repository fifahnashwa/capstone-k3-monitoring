...
class DetectionWorkerService
{
    ...
    public function getWorkerCount(int $cameraId): int
    {
        try {
            $response = Http::withHeaders($this->headers())
                ->timeout(2)
                ->get("{$this->baseUrl}/api/cameras/{$cameraId}/workers");
            if ($response->successful()) {
                return (int) $response->json('worker_count', 0);
            }
        } catch (\Exception) {}
        return 0;
    }

    public function uploadModel(\Illuminate\Http\UploadedFile $file): array
    {
        try {
            $stream = fopen($file->getRealPath(), 'r');

            $response = Http::withHeaders(['X-Service-Key' => $this->serviceKey])
                ->timeout(300)
                ->attach('file', $stream, $file->getClientOriginalName())
                ->post("{$this->baseUrl}/api/models/upload");
            ...
            return ['success' => false, 'message' => $response->json('detail', 'Upload gagal di worker.')];
        } catch (\Exception $e) {
            Log::error("DetectionWorker uploadModel error: {$e->getMessage()}");
            return ['success' => false, 'message' => 'Detection worker tidak dapat dihubungi: ' . $e->getMessage()];
        }
    }

    /**
     * Kembalikan null jika worker tidak dapat dihubungi, array (bisa kosong) jika berhasil.
     */
    public function listModels(): ?array
    {
        try {
            ...
            Log::debug("DetectionWorker listModels error: {$e->getMessage()}");
            return null;
        }
    }

    public function trainFaceModel(string $empId, string $empName, array $imagesB64): array
    {
        try {
            $response = Http::withHeaders($this->headers())
                ->timeout(120)
                ->post("{$this->baseUrl}/api/worker-faces/train", [
                    'employee_id'   => $empId,
            ...
            return ['success' => false, 'message' => $response->json('detail', 'Training gagal di worker.')];
        } catch (\Exception $e) {
            Log::error("DetectionWorker trainFaceModel error: {$e->getMessage()}");
            throw $e;
        }
    }

    public function deleteFaceModel(string $empId): bool
    {
        try {
            $response = Http::withHeaders($this->headers())
                ->timeout($this->timeout)
                ->delete("{$this->baseUrl}/api/worker-faces/{$empId}");

            return $response->successful();
        } catch (\Exception $e) {
            Log::warning("DetectionWorker deleteFaceModel error: {$e->getMessage()}");
            return false;
        }
    }
