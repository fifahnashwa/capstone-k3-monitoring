...
<div class="grid grid-cols-3 gap-5">
    ...
    <div class="col-span-2">
        <div id="result-placeholder" class="bg-white rounded-xl border border-gray-100 p-10 text-center text-sm text-gray-400 h-full flex items-center justify-center">
            Isi parameter di kiri, lalu klik Generate Laporan.
        </div>
        <div id="result-loading" class="hidden bg-white rounded-xl border border-gray-100 p-10 text-center text-sm text-gray-400">
            Membuat laporan...
        </div>
...
function renderResult(d) {
    ...
    document.getElementById('r-zones').innerHTML = (d.by_zone ?? []).map(z => `
        <div class="flex items-center gap-3">
            ...
            <div class="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                <div class="bg-blue-500 h-2 rounded-full" style="width:${d.total_violations ? Math.round(z.total/d.total_violations*100) : 0}%"></div>
            </div>
    ...
    document.getElementById('r-shifts').innerHTML = (d.by_shift ?? []).map(s => `
        <div class="flex items-center gap-3">
            ...
            <div class="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
                <div class="bg-green-500 h-2 rounded-full" style="width:${d.total_violations ? Math.round(s.total/d.total_violations*100) : 0}%"></div>
            </div>
