  ...
  <div class="bg-white rounded-2xl w-full max-w-2xl shadow-2xl mx-4" onclick="event.stopPropagation()">
    ...
    <div class="px-6 py-4 space-y-4 max-h-[65vh] overflow-y-auto">
      ...
      <div id="tab-ai" class="tab-content space-y-3">
        <div>
            <label class="block text-xs text-gray-500 mb-1">Path Model AI (.onnx)</label>
            <input type="text" id="f-model-path"
                   class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                   placeholder="/storage/models/best.onnx">
        </div>
        <div class="grid grid-cols-2 gap-3">
            <div>
...
function openModal(camera = null) {
    ...
    document.getElementById('f-class-map').value    = JSON.stringify(camera?.class_mapping ?? {
        person_id: 6, helmet_ids: [0], vest_ids: [2], boots_ids: [3]
    }, null, 2);
