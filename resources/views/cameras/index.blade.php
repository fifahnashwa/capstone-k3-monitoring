...
@push('head')
<style>
    .tab-btn.active { background:#eff6ff; color:#1d4ed8; border-color:#bfdbfe; }
    ...
    .conn-ok   { background:#f0fdf4; border:1px solid #86efac; }
    .conn-fail { background:#fef2f2; border:1px solid #fca5a5; }
</style>
@endpush
...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-lg font-semibold text-gray-900">Manajemen Kamera</h1>
        <p class="text-xs text-gray-400 mt-0.5">Kelola semua kamera pengawas APD</p>
    </div>
    <div class="flex gap-2">
        <a href="{{ route('cameras.monitoring') }}"
           class="px-4 py-2 text-sm border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 flex items-center gap-1.5">
        ...
        <button onclick="openModal()" class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-1.5">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
{{-- FILTER BAR --}}
<div class="bg-white rounded-xl border border-gray-100 p-3 mb-4 flex flex-wrap gap-3 items-center">
    <input type="text" id="search" placeholder="Cari nama, IP, atau zona..."
           class="flex-1 min-w-48 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-100"
...
{{-- PAGINATION --}}
<div id="pagination" class="hidden flex items-center justify-between mt-4 text-sm text-gray-500"></div>
...
<div id="modal" class="hidden fixed inset-0 z-40 flex items-center justify-center overflow-y-auto py-6" style="background:rgba(0,0,0,0.4)">
  <div class="bg-white rounded-2xl w-full max-w-2xl shadow-2xl mx-4" onclick="event.stopPropagation()">

    {{-- Modal Header --}}
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <h2 id="modal-title" class="text-base font-semibold text-gray-900">Tambah Kamera</h2>
    ...
    <div class="px-6 py-4 space-y-4 max-h-[65vh] overflow-y-auto">
      ...
      <div id="tab-info" class="tab-content active space-y-3">
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs text-gray-500 mb-1">Nama Kamera <span class="text-red-500">*</span></label>
                <input type="text" id="f-name" maxlength="100"
                       class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                       placeholder="Kamera Forklift 1">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Zona <span class="text-red-500">*</span></label>
                <select id="f-zone-modal" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-blue-100 focus:outline-none"></select>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs text-gray-500 mb-1">DVR Channel</label>
            ...
            <div>
                ...
                <select id="f-active" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-blue-100 focus:outline-none">
                    <option value="1">Aktif</option>
                    <option value="0">Nonaktif</option>
                </select>
            </div>
        ...
        <div>
            <label class="block text-xs text-gray-500 mb-1">Deskripsi / Lokasi Detail</label>
            <textarea id="f-desc" rows="2"
                      class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none resize-none"
      ...
      <div id="tab-network" class="tab-content space-y-3">
        <div class="grid grid-cols-3 gap-3">
            <div class="col-span-2">
                <label class="block text-xs text-gray-500 mb-1">IP Address <span class="text-red-500">*</span></label>
                <input type="text" id="f-ip"
                       class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                       placeholder="10.134.28.100">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Port ONVIF</label>
        ...
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs text-gray-500 mb-1">Username Kamera <span class="text-red-500">*</span></label>
                <input type="text" id="f-username" autocomplete="off"
                       class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                       placeholder="TC70BKelompok2A2">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1" id="pass-label">Password <span class="text-red-500">*</span></label>
                <div class="flex gap-1">
                    <input type="password" id="f-password" autocomplete="new-password"
                           class="flex-1 text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none"
                           placeholder="••••••••••">
                    <button type="button" id="pass-toggle-btn" class="hidden px-2 py-1 text-xs border border-gray-200 rounded-lg text-blue-600 hover:bg-blue-50 whitespace-nowrap"
        ...
        <div class="grid grid-cols-3 gap-3">
            <div>
                <label class="block text-xs text-gray-500 mb-1">Port RTSP</label>
                <input type="number" id="f-port-rtsp" value="554" min="1" max="65535"
                       class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Path Stream</label>
                <select id="f-rtsp-path" onchange="checkCustomPath()"
                        class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-blue-100 focus:outline-none">
            ...
            <div>
                ...
                <select id="f-rtsp-transport" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:ring-2 focus:ring-blue-100 focus:outline-none">
        ...
        <div id="custom-path-wrap" class="hidden">
            <label class="block text-xs text-gray-500 mb-1">Custom Path</label>
            <input type="text" id="f-rtsp-path-custom" placeholder="/custom/path"
                   class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none">
        </div>
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs text-gray-500 mb-1">Connection Timeout (detik)</label>
                <input type="number" id="f-timeout" value="10" min="1" max="60"
                       class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none">
            </div>
        </div>
        <button onclick="runConnectionTest()" id="btn-test-conn"
                class="w-full py-2 text-sm border-2 border-blue-200 text-blue-700 rounded-lg hover:bg-blue-50 flex items-center justify-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div id="tab-ai" class="tab-content space-y-3">
        ...
        <div>
            <label class="block text-xs text-gray-500 mb-1">Path Model Aktif (.pt / .onnx)</label>
            <div class="flex gap-1.5">
                ...
                <button type="button" onclick="document.getElementById('f-model-path').value=''"
                        class="px-2 text-gray-400 hover:text-red-500 border border-gray-200 rounded-lg" title="Hapus">✕</button>
            </div>
        </div>
        ...
        <div class="border border-gray-100 rounded-xl overflow-hidden">
            <div class="flex items-center justify-between px-3 py-2 bg-gray-50 border-b border-gray-100">
                <span class="text-xs font-medium text-gray-600">Model Tersedia di Worker</span>
                <button type="button" onclick="loadModelList()" class="text-xs text-blue-600 hover:underline">↻ Refresh</button>
            </div>
            <div id="model-list" class="max-h-40 overflow-y-auto text-sm text-gray-400 py-3 text-center">
                Klik Refresh untuk memuat daftar model.
            </div>
        </div>

        {{-- Upload model baru --}}
        <div class="border border-dashed border-blue-100 rounded-xl p-3 bg-blue-50/30">
            <div class="text-xs font-medium text-gray-600 mb-2">Upload Model Baru ke Worker</div>
            <div class="flex gap-2">
                <label class="flex-1 cursor-pointer">
                    <input type="file" id="model-file-input" accept=".pt,.onnx,.zip" class="hidden"
                           onchange="onModelFileChosen()">
                    <div class="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg bg-white text-sm text-gray-500 hover:bg-gray-50 truncate">
                ...
                <button type="button" id="btn-upload-model" onclick="doUploadModel()"
                        class="px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
                        disabled>Upload</button>
            </div>
            {{-- Progress bar --}}
            <div id="upload-progress" class="hidden mt-2 space-y-1">
                <div class="flex items-center gap-2">
                    <div class="flex-1 bg-white rounded-full h-1.5 border border-gray-100">
                        <div id="upload-bar" class="bg-blue-500 h-1.5 rounded-full transition-all duration-150" style="width:0%"></div>
                    </div>
                    <span id="upload-pct" class="text-xs text-gray-500 w-8 text-right">0%</span>
                </div>
                <div id="upload-status" class="text-xs text-gray-500"></div>
            </div>
            <p class="text-xs text-gray-400 mt-1.5">Format: .pt (PyTorch), .onnx, atau .zip yang berisi file .pt</p>
        </div>

        {{-- Detection settings --}}
        <div class="grid grid-cols-2 gap-3">
            <div>
                <label class="block text-xs text-gray-500 mb-1">Detection Size</label>
                <input type="number" id="f-det-size" value="640" min="320" max="1280" step="32"
                       class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none">
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Proses Setiap N Frame</label>
        ...
        <div class="slider-wrap">
            <label class="block text-xs text-gray-500 mb-1">
                ...
                <span id="conf-val" class="font-medium text-blue-600">0.40</span>
            </label>
        ...
        <div>
            <label class="block text-xs text-gray-500 mb-1">Class ID Mapping (JSON)</label>
            <textarea id="f-class-map" rows="5"
                      class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 font-mono text-xs focus:ring-2 focus:ring-blue-100 focus:outline-none resize-none">{
  "person_id": 5,
  "helmet_ids": [1],
  "vest_ids": [6],
  "boots_ids": [0],
  ...
  "no_boots_ids": [2]
            ...
            <p class="text-xs text-gray-400 mt-1">Model 7-kelas: boots(0) helmet(1) no_boots(2) no_helmet(3) no_vest(4) person(5) vest(6)</p>
        </div>
        <div class="grid grid-cols-2 gap-3 items-center">
            ...
            <div>
                <label class="block text-xs text-gray-500 mb-1">Cooldown Screenshot (detik)</label>
        ...
        <div id="ptz-fields" class="hidden space-y-3">
            <div class="slider-wrap">
                <label class="block text-xs text-gray-500 mb-1">
                    ...
                    <span id="ptz-speed-val" class="font-medium text-blue-600">0.6</span>
                </label>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs text-gray-500 mb-1">Durasi Gerakan (detik)</label>
                    <input type="number" id="f-ptz-duration" value="0.4" min="0.1" max="5.0" step="0.1"
                           class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-100 focus:outline-none">
                </div>
            </div>
            <div>
                <label class="block text-xs text-gray-500 mb-1">Preset Positions (JSON)</label>
                <textarea id="f-ptz-presets" rows="4"
                          class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 font-mono text-xs focus:ring-2 focus:ring-blue-100 focus:outline-none resize-none"
                          placeholder='[{"name":"Pintu Masuk","pan":0.5,"tilt":0.2,"zoom":1.0}]'></textarea>
                <p class="text-xs text-gray-400 mt-1">Format: array of {name, pan, tilt, zoom}</p>
            </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
        <button onclick="closeModal()" class="px-4 py-2 text-sm border border-gray-200 text-gray-600 rounded-lg hover:bg-white">
            Batal
        </button>
        <div class="flex gap-2">
            <button onclick="saveCamera(true)" id="btn-save-test"
                    class="px-4 py-2 text-sm border border-blue-200 text-blue-700 rounded-lg hover:bg-blue-50">
                Simpan & Test
            </button>
...
<div id="del-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center" style="background:rgba(0,0,0,0.4)">
    <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl mx-4">
        <h3 class="font-semibold text-gray-900 mb-2">Hapus Kamera</h3>
        <p class="text-sm text-gray-500 mb-5">Kamera <strong id="del-name"></strong> akan dihapus. Aksi ini tidak bisa dibatalkan.</p>
        <div class="flex gap-2">
            <button onclick="confirmDelete()" class="flex-1 py-2 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700">Hapus</button>
            <button onclick="document.getElementById('del-modal').classList.add('hidden')"
                    class="flex-1 py-2 text-sm border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50">Batal</button>
        </div>
    </div>
</div>
@endsection
...
function renderCameras(cameras) {
    ...
    tbody.innerHTML = cameras.map(c => `
        <tr class="hover:bg-gray-50">
            <td class="px-4 py-3">
                <div class="font-medium text-gray-800">${escHtml(c.name)}</div>
            </td>
            <td class="px-4 py-3 text-gray-500 text-xs">${escHtml(c.zone_name ?? '—')}</td>
            <td class="px-4 py-3 text-gray-400 text-xs font-mono">${escHtml(c.dvr_channel ?? '—')}</td>
            <td class="px-4 py-3 text-gray-500 text-xs font-mono">${escHtml(c.ip_address ?? '—')}</td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                    ${IS_ADMIN ? `<button onclick='editCamera(${JSON.stringify(c)})' class="text-xs text-blue-600 hover:underline">Edit</button>` : ''}
                    <a href="/cameras/${c.id}" class="text-xs text-gray-500 hover:underline">Detail</a>
                    ${IS_ADMIN ? `
                    <button onclick="testConn(${c.id})" class="text-xs text-indigo-500 hover:underline">Test</button>
                    <button onclick="openDelete(${c.id}, '${escHtml(c.name)}')" class="text-xs text-red-500 hover:underline">Hapus</button>
                    ` : ''}
                </div>
            </td>
        </tr>
...
function renderPagination(meta) {
    ...
    p.innerHTML = `
        ...
        <div class="flex gap-1">
            ${meta.page > 1 ? `<button onclick="loadData(${meta.page - 1})" class="px-3 py-1 border border-gray-200 rounded-lg hover:bg-gray-50">← Prev</button>` : ''}
            ${meta.page < meta.last_page ? `<button onclick="loadData(${meta.page + 1})" class="px-3 py-1 border border-gray-200 rounded-lg hover:bg-gray-50">Next →</button>` : ''}
        </div>
    `;
}
...
function openModal(camera = null) {
    ...
    if (camera && camera.has_password) {
        ...
        passLabel.innerHTML   = 'Password <span class="text-gray-400 font-normal">(tersimpan)</span>';
    } else {
        passInput.value       = '';
        passInput.placeholder = '••••••••••';
        passInput.disabled    = false;
        passToggle.classList.add('hidden');
        passLabel.innerHTML   = 'Password <span class="text-red-500">*</span>';
    }
...
function showConnResult(d) {
    ...
    if (d?.onvif) {
        html += `<div class="text-xs mt-1">ONVIF: ${d.onvif.ok ? '✅ Connected' : '❌ ' + (d.onvif.error ?? 'Gagal')}</div>`;
        if (d.onvif.latency_ms) html += ` <span class="text-gray-400">(${d.onvif.latency_ms}ms)</span>`;
    }
    if (d?.rtsp) {
        html += `<div class="text-xs mt-0.5">RTSP: ${d.rtsp.ok ? '✅ Stream tersedia' : '❌ ' + (d.rtsp.error ?? 'Gagal')}`;
        if (d.rtsp.resolution) html += ` <span class="text-gray-400">${d.rtsp.resolution}</span>`;
        if (d.rtsp.fps)        html += ` <span class="text-gray-400">${d.rtsp.fps}fps</span>`;
        html += '</div>';
    }
...
async function loadModelList() {
    ...
    list.innerHTML = '<div class="py-3 text-xs text-gray-400">Memuat...</div>';
    try {
        ...
        if (!_modelCache.length) {
            list.innerHTML = '<div class="py-3 px-3 text-xs text-gray-400">Belum ada model di folder <code>detection_worker/models/</code>. Upload file di bawah.</div>';
            return;
        }
        ...
        list.innerHTML = _modelCache.map((m, i) => {
            ...
            return `
            <div class="flex items-center justify-between px-3 py-2 hover:bg-blue-50 cursor-pointer border-b border-gray-50 last:border-0"
                 onclick="selectModelByIdx(${i})">
                <div class="min-w-0">
                    <div class="text-sm font-medium text-gray-800 truncate">${escHtml(m.name)}</div>
                    <div class="text-xs text-gray-400">${m.size_mb} MB</div>
                </div>
